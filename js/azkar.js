...
let all = {
    ...
    ad3eia: [
      {
        ...
        description: "",
        reference: ". [البقرة - 21].",
        content: " رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ. [البقرة - 21]. "
      },
      {
        category: "أدعية قرآنية",
        count: "1",
    ...
    elanbia: [
      ...
      },
      {
        ...
        content: " فَاطِرَ السَّمَاوَاتِ وَالْأَرْضِ أَنتَ وَلِيِّي فِي الدُّنْيَا وَالْآخِرَةِ ۖ تَوَفَّنِي مُسْلِمًا وَأَلْحِقْنِي بِالصَّالِحِينَ. [يوسف - 11]. "
      },
      {
        category: "أدعية الأنبياء",
        count: "1",
  ...
  let azkarall = document.getElementById("azkarall");
  let content = document.getElementById("content");

  function add() {
    const elements = ['saba7', 'masaa', 'esteqaz', 'elnoom', 'b3delsalah', 'ad3eia', 'elanbia', 'tasbeh'];
    const buttons = elements.map(element => `<div onclick = clickToShow('${element}') class="fs-3 p-3 rounded-3 border border-3 border-secondary m-2 text-center fw-semibold hoversorah w-25">${all[element][0].category}</div>`).join('');
    azkarall.innerHTML = buttons;
  }
  
  function clickToShow(element) {
    const items = all[element];
    azkarall.innerHTML = '';
    for (let i = 0; i < items.length; i++) {
      content.innerHTML += `
        <div class="fs-3 bg-white p-3 rounded rouneded-3 my-3" dir="rtl">${items[i].content}</div>
        <div class="text-center my-5"><button onclick="countless('${element}', '${i}')" class="btn btn-dark py-2 px-4 fs-4 btn-counter">${items[i].count}</button></div>
      `;
    }
  }
  ...
  function countless(element, i) {
    let btns = document.getElementsByClassName("btn-counter");
    if (all[element][i].count > 1) {
      all[element][i].count--;
      btns[i].innerHTML = all[element][i].count;
    } else {
      btns[i].innerHTML = "Done";
      scrollBy(0,220)
      btns[i].classList.add("bg-info2", "fw-bold", "text-dark")
